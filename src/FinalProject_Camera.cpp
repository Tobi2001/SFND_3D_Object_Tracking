...
int main(int argc, const char* argv[])
{
    ...
    RT.at<double>(0, 0) = 7.533745e-03; RT.at<double>(0, 1) = -9.999714e-01; RT.at<double>(0,
        2) = -6.166020e-04; RT.at<double>(0, 3) = -4.069766e-03;
    RT.at<double>(1, 0) = 1.480249e-02; RT.at<double>(1, 1) = 7.280733e-04; RT.at<double>(1,
        2) = -9.998902e-01; RT.at<double>(1, 3) = -7.631618e-02;
    RT.at<double>(2, 0) = 9.998621e-01; RT.at<double>(2, 1) = 7.523790e-03; RT.at<double>(2,
        2) = 1.480755e-02; RT.at<double>(2, 3) = -2.717806e-01;
    ...
    std::vector<std::string> detectorTypes = {"SHITOMASI", "HARRIS", "BRISK", "ORB", "AKAZE", "SIFT"};
    ...
    for (const auto& detectorType : detectorTypes)
    {
        ...
        {
            ss << detectorType << " " << descriptorType << std::endl;

            if ((descriptorType == "AKAZE" && detectorType != descriptorType) ||
                (descriptorType == "ORB" && detectorType == "SIFT"))
            {
                ss << "Incompatible descriptor/detector pair" << std::endl << std::endl;
                continue;
            }

            double ttcDiffSum = 0.0;
            int ttcDiffCount = 0;
            ...
            //string detectorType = "SHITOMASI"; // SHITOMASI, HARRIS, BRISK, ORB, AKA, SIFT
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
            {
                ...
                detectObjects(dataBuffer.at(0).cameraImg,
                    dataBuffer.at(0).boundingBoxes, confThreshold, nmsThreshold,
                ...
                dataBuffer.at(0).lidarPoints = lidarPoints;
                ...
                clusterLidarWithROI(dataBuffer.at(0).boundingBoxes,
                ...
                cv::Mat imgGray;
                cv::cvtColor(dataBuffer.at(0).cameraImg, imgGray, cv::COLOR_BGR2GRAY);
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    if (descriptorType == "SIFT" || descriptorType == "AKAZE")
                    {
                        descType = "DES_HOG";
                    }
                    ...
                    matchBoundingBoxes(matches, bbBestMatches, dataBuffer.at(1), dataBuffer.at(0)); // associate bounding boxes between current and previous frame using keypoint matches
                    //// EOF STUDENT ASSIGNMENT

                    // store matches in current data frame
                    dataBuffer.at(0).bbMatches = bbBestMatches;
                    ...
                    for (auto it1 = dataBuffer.at(0).bbMatches.begin(); it1 != dataBuffer.at(0).bbMatches.end();
                        ++it1)
                    {
                        // find bounding boxes associates with current match
                        BoundingBox* prevBB, * currBB;
                        for (auto it2 = dataBuffer.at(0).boundingBoxes.begin();
                            it2 != dataBuffer.at(0).boundingBoxes.end(); ++it2)
                        {
                        ...
                        for (auto it2 = dataBuffer.at(1).boundingBoxes.begin();
                            it2 != dataBuffer.at(1).boundingBoxes.end(); ++it2)
                        {
                        ...
                        if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                        {
                            ...
                            double ttcCamera;
                            clusterKptMatchesWithROI(*currBB, dataBuffer.at(1).keypoints,
                                dataBuffer.at(0).keypoints, dataBuffer.at(0).kptMatches);

                            computeTTCCamera(dataBuffer.at(1).keypoints,
                                dataBuffer.at(0).keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                            //// EOF STUDENT ASSIGNMENT

                            double ttcDiff = std::fabs(ttcLidar - ttcCamera);
                            if (!std::isnan(ttcDiff))
                            {
                                ++ttcDiffCount;
                                ttcDiffSum += ttcDiff;
                            }
                            ss << " " << ttcLidar << " " << ttcCamera << " " << ttcDiff << std::endl;


                            bVis = false;
                            if (bVis)
                            {
                                cv::Mat visImg = dataBuffer.at(0).cameraImg.clone();
                                showLidarImgOverlay(visImg, currBB->lidarPoints, P_rect_00, R_rect_00, RT, &visImg);
                                cv::rectangle(visImg, cv::Point(currBB->roi.x, currBB->roi.y),
                                    cv::Point(currBB->roi.x + currBB->roi.width,
                                    currBB->roi.y + currBB->roi.height), cv::Scalar(
                                        0, 255,
                                        0), 2);
            ...
            ss << "TTC_average: " << (ttcDiffCount > 0 ? ttcDiffSum / ttcDiffCount : 0.0) << std::endl << std::endl;
        }
    }
